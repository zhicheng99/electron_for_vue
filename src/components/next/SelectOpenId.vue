...
<script>
	export default{
		components:{},
		props:['fieldName','openId','placeHolder'],
		data(){
			return {
				placeHolder:'请输入执行人',
				loading:false,
		...
		methods:{ 
			...
			handleSelectExecutionPerson:function(){
				...
				this.$nextTick(()=>{
					...
					let arr = this.fieldName.split('||');
					this.$emit('update:fieldName',arr[0]);
					this.$emit('update:openId',arr[1]);
