   ...
   <!-- label可以编辑 非必添-->
		       ...
		       <!--  <el-form-item
		        ...
		        <el-form-item>
			        ...
			        <el-button size="small" type="warning" v-if="form.state!='1'" @click="submit('2')">暂 存</el-button>
			...
			createVideo:function(data){
				...
				dic.VideoLabel().then(({code,obj})=>{
					(code == '00') && (()=>{
						...
						//根据标签中label做对应 重置videoUrl和labelName
						tmp.forEach(item=>{
							data.videos.forEach(v=>{
								if(item.label == v.label){
									item.videoUrl = v.videoUrl
									item.labelName = v.labelName!== null?v.labelName:item.labelName
								}
							})
						})
				...
				this.$post('/admin/village/query',{id:id}).then(({code,obj})=>{
					(code == '00') && (()=>{
						...
						this.form = {
							...
							...obj,
							...{
								pos:(obj.lng && obj.lat) ?obj.lng+','+obj.lat:''
							}
						}
