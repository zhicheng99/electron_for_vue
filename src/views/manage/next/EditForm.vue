		...
		<el-dialog
		    ...
		    <el-form :model="form" :rules="formRules" ref="form" :label-width="formLabelWidth">
		        ...
		        <el-form-item>
		            ...
		            <el-button size="small" type="primary" v-if="form.state =='2'" @click="onSubmit('2')" :loading="loading2">暂存</el-button>
		            <el-button size="small" type="primary" v-if="form.state =='2' || form.state =='1'" @click="onSubmit('1')" :loading="loading1">提交发布</el-button>


		            <el-button size="small" type="primary" v-if="form.state =='2' || form.state =='1' || form.state =='0'" @click="onSubmit('0')" :loading="loading0">提交不发布</el-button>
		        </el-form-item>
		    </el-form>
...
<script>
	...
	export default{
		...
		props:['isShow','modelId','callBack','rowId'],
		data(){
			return {
				show:false,
		...
		methods:{
			...
			onSubmit:function(sign){
				...
				this.$refs.form.validate(valid=>{
					...
					if(valid  && (sign !== '2')){
						...
						this.$post('/admin/resources/edit',this.form).then(({code})=>{
			...
			detail:function(v){
				this.$post('/admin/resources/details',{uid:v}).then(({code,obj})=>{
					(code == '00') && (()=>{
						...
						this.form = {
							...
							...{
								...
								multiWordGroup:(()=>{
									...
									if(obj.type == '0' || obj.type == '2'){ //文章或文件
										let tmp = [];
								...
								referenceGroup:(()=>{
									...
									if(typeof obj.reference !='undefined' &&  obj.reference !=''){
										...
										if(obj.reference.indexOf('|#|')>-1){
											let tmp = [];
											let arr = obj.reference.split('|#|');
											arr.forEach(item=>{
												tmp.push({
													url:item
												})
