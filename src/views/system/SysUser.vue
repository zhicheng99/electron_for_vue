		...
		<el-row class="m-b-20">
			<el-col :span="24" class="toolbar">
				<el-form :model="filters" :inline="true" ref="filters">
					 <el-form-item  prop="s_type">
						 <el-input v-model="filters.s_key" placeholder="请输入登录名/邮箱/手机号" size="small"></el-input>
					 </el-form-item>  
					 <el-form-item>
	...
	export default {
		...
		methods: {  
			...
			getUsers() {
				...
				this.$post('/admin/system/user/list',params).then((res)=>{
			...
			handleDel: function (id) {
				this.$confirm('确认删除该用户吗?', '提示', {
					...
					this.$post('/admin/system/user/delete',para).then(({code,obj})=>{
				...
				this.$refs.editForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$post('/admin/system/user/edit2',para).then(({code,obj})=>{
				...
				this.$refs.addForm.validate((valid) => {
					if (valid) {
						this.$confirm('确认提交吗？', '提示', {}).then(() => {
							...
							this.$post('/admin/system/user/add2',para).then(({code,obj})=>{
